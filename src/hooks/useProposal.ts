...
export function useProposal({ proposal }: Props) {
  ...
  const formatAccountsByChain = useCallback(
    (proposal: Proposal, accounts: Account[]) => {
  ...
  const createChains = useCallback((accountsByChain: AccountsInChain[]) => {
  ...
  const createNamespaces = useCallback((): Record<
    string,
  ...
  > => {
    ...
    return {
      eip155: {
        ...
        events:
          proposal.params.requiredNamespaces[SUPPORTED_NAMESPACE.eip155].events,
  ...
  }, [addSession, createNamespaces, navigate, proposal.id])
  ...
  const addNewAccount = useCallback(
    ...
    [addAccount, closeTransport, initWalletApiClient],
  )
