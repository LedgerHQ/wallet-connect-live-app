...
const useSessionsStore = create<SessionsState>()(
  persist(
    (set) => ({
      ...
      setLastSessionVisited: (session) => set(() => ({ lastSessionVisited: session })),
      addSession: (session) => set((state) => ({ sessions: [...state.sessions, session] })),
      addSessions: (sessions) => set(() => ({ sessions: sessions })),
      clearSessions: () => set(() => ({ sessions: [], lastSessionVisited: null })),
...
const sessionSelector = {
  ...
  selectLastSession: (state: SessionsState): Session | null => state.lastSessionVisited,
